...
5. **Keep the project open.** Develop in public, accept scientific and engineering contributions, and publish original project work under MIT where we have the rights to do so.
6. **Offer a simple interface once forecasts are trustworthy.** Make selected fields and run metadata easy to inspect, alongside raw output and enough provenance to interpret it.
...
- Rewrite atmospheric dynamics from scratch or infer them solely from published forecast output when source code is available.
